...
void deleteMeshInstanceByRay(Mesh& mesh, GLFWwindow* window, const GLFWvidmode* mode, Camera* camera) {
    if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_RIGHT) == GLFW_PRESS) {
        ...
        for (int i = 0; i < mesh.ibo.aabbs.size(); ++i) {
            float tMin = 0.1f;
            float tMax = 1000.0f;
            if (mesh.ibo.aabbs[i].rayIntersects(camera->Position, rayWorld, tMin, tMax)) {
                mesh.ibo.deleteInstance(i);
            }
        }
    }
}
...
Ray::Ray() : rayShader("shaders/ray_vert.glsl", "shaders/ray_frag.glsl") {}
